...
def train_model(dataloaders, dataset_size, adversarial_loss, L1_loss, pixelwise_loss,
    ...
    for epoch in range(num_epochs):
        ...
        for i, (masked_imgs, imgs) in tqdm(enumerate(dataloaders['train'])):
            ...
            g_adv= 0.5 * g_adv_local + 0.5 * g_adv_global

            g_l1 = 0.5 * L1_loss(gen_imgs, imgs) + 0.5 * L1_loss(gen_parts, masked_parts)

            g_pixel = 0.5 * pixelwise_loss(torch.sigmoid(gen_imgs), imgs*0.5+0.5) +\
                      0.5 * pixelwise_loss(torch.sigmoid(gen_parts), masked_parts*0.5+0.5)
            # Total loss
            g_loss = 0.001 * g_adv + g_l1 + 0.005 * g_pixel
    ...
    torch.save(generator, 'generator_model_end.pth')
    torch.save(global_discriminator, 'global_discriminator_end.pth')
    torch.save(local_discriminator, 'local_discriminator_end.pth')
    with open('result/result_{}.json'.format(time.strftime("%Y_%m_%d_%H", time.localtime(time.time()))),
              'w') as f:
        json.dump(loss_dict, f)
