...
def train_model(dataloaders, dataset_size, adversarial_loss, L2_loss, pixelwise_loss,
    ...
    for epoch in range(num_epochs):
        ...
        for i, (masked_imgs, imgs) in tqdm(enumerate(dataloaders['train'])):
            ...
            g_l2 = L2_loss(gen_imgs, imgs)
            g_pixel = pixelwise_loss(torch.sigmoid(gen_imgs), imgs*0.5+0.5)
            # Total loss
            g_loss = 0.001 * g_adv_local + 0.001 * g_adv_global + 0.999 * g_l2 + 0.005 * g_pixel
            ...
            running_loss5 += g_l2.item() * imgs.size(0)
        ...
        print(
            "[D local loss: %f] [D global loss: %f] [G local adv: %f, G global adv: %f, L2: %f, pixel: %f]"
        ...
        loss_dict['g_l2'].append(epoch_loss5)
...
if __name__ == '__main__':
    ...
    train_model(dataloaders, dataset_size, adversarial_loss, L2_loss, pixelwise_loss,
                generator, global_discriminator, local_discriminator,
                optimizer_G, optimizer_Dg, optimizer_Dl, num_epochs=Config['num_epochs'])
